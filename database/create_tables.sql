...
(
    ...
    CONSTRAINT FK_DayPlans_TripPlans FOREIGN KEY (TripPlanId)
        REFERENCES dbo.TripPlans(Id) ON DELETE CASCADE,
    CONSTRAINT CK_DayPlans_DayNumber CHECK (DayNumber > 0)
);
GO

CREATE INDEX IX_DayPlans_TripPlanId ON dbo.DayPlans(TripPlanId);
GO
CREATE UNIQUE INDEX UX_DayPlans_TripPlanId_DayNumber ON dbo.DayPlans(TripPlanId, DayNumber);
GO
CREATE INDEX IX_TripPlans_CreatedAt ON dbo.TripPlans(CreatedAt DESC);
GO
